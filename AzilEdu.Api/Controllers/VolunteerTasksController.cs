...
public class VolunteerTasksController : ControllerBase
{
    ...
    {
        var error = await ValidateTask(request);
    ...
    private async Task<string?> ValidateTask(SaveVolunteerTaskDto request)
    {
        
        var statusName = await _context.VolunteerTaskStatuses
            .Where(s => s.Id == request.VolunteerTaskStatusId)
            .Select(s => s.Name)
            .FirstOrDefaultAsync();

        
        if (statusName == "Završeno" && request.CompletedAt is null)
            return "Ako je status 'Završeno', datum završetka mora biti ispunjen.";

        
        if (statusName != "Završeno" && request.CompletedAt is not null)
            return "Datum završetka smije biti ispunjen samo ako je status 'Završeno'.";
